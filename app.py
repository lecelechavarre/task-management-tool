...
class TodoApp:
    ...
    def _load_archived_tasks(self):
        return storage.load_tasks(ARCHIVE_PATH) if os.path.exists(ARCHIVE_PATH) else []

    def _load_active_tasks(self):
        all_tasks = storage.load_tasks(TASKS_PATH)
        return [t for t in all_tasks if getattr(t, 'status', 'pending') != 'archived']
    ...
    def _setup_style(self):
        ...
        self.style.configure("Card.TFrame",
            background=APP_COLORS["bg_card"], relief="solid", borderwidth=1,
    ...
    def _build_ui(self):
        ...
        self.root.columnconfigure(2, weight=0, minsize=200)
        ...
        ttk.Label(search_frame, text="🔍", font=("Segoe UI", 10)).grid(row=0, column=0, padx=(0,8))
        self.search_var = tk.StringVar()
        search_entry = ttk.Entry(search_frame, textvariable=self.search_var, font=("Segoe UI", 10))
        search_entry.grid(row=0, column=1, sticky="ew")
        self.search_var.trace_add("write", lambda *_: self._render_tasks())
        ...
        add_button.grid(row=0, column=3, padx=(12,6))

        # Finished counter
        finished_frame = ttk.Frame(top, style="Finished.TFrame", padding=(12, 6))
        finished_frame.grid(row=0, column=4, padx=(6,0))
        self.finished_count_label = ttk.Label(finished_frame, 
                                             text="✓ 0 Finished",
                                             font=("Segoe UI", 10, "bold"),
                                             foreground=APP_COLORS["success"])
        self.finished_count_label.pack()

        # --- Main grid: archive | tasks+overview | finished ---
        self.root.columnconfigure(0, weight=1, minsize=120)   # Archive smaller
        self.root.columnconfigure(1, weight=8, minsize=700)   # Tasks bigger
        self.root.columnconfigure(2, weight=1, minsize=120)   # Finished smaller
        self.root.rowconfigure(1, weight=1)

        left = ttk.Frame(self.root, padding=(6,6))
        left.grid(row=1, column=0, sticky="nsew")
        center = ttk.Frame(self.root, padding=(12,6))
        center.grid(row=1, column=1, sticky="nsew")
        right = ttk.Frame(self.root, padding=(6,6))
        right.grid(row=1, column=2, sticky="nsew")

        # --- Center: Overview ABOVE Task list ---
        overview_card = ttk.Frame(center, style="Card.TFrame", padding=3)
        overview_card.grid(row=0, column=0, sticky="ew", padx=6, pady=(0,6))
        ttk.Label(overview_card, text="Overview", font=("Segoe UI", 12, "bold")).grid(row=0, column=0, sticky="w")
        self.stats_label = ttk.Label(overview_card, text="", style="Muted.TLabel", wraplength=400)
        self.stats_label.grid(row=1, column=0, sticky="w", pady=(8,0))

        # Task list (scrollable)
        self.task_canvas = tk.Canvas(center, borderwidth=0, highlightthickness=0, bg=APP_COLORS["bg_main"])
        self.task_scroll = ttk.Scrollbar(center, orient="vertical", command=self.task_canvas.yview)
        ...
        self.task_scroll.grid(row=1, column=1, sticky="ns")
        center.rowconfigure(1, weight=1)
        center.columnconfigure(0, weight=1)

        # --- Left: Archive ---
        archive_card = ttk.Frame(left, style="Archive.TFrame", padding=6)
        archive_card.grid(row=0, column=0, sticky="nsew", padx=6)
        ...
        self.archive_count_label = ttk.Label(archive_header, text="", style="Archive.TLabel")
        ...
        self.archive_canvas = tk.Canvas(self.archive_frame, borderwidth=0,
                                      highlightthickness=0, bg=APP_COLORS["bg_card"])
        self.archive_scroll = ttk.Scrollbar(self.archive_frame, orient="vertical", command=self.archive_canvas.yview)
        self.archive_list_frame = ttk.Frame(self.archive_canvas)
        self.archive_list_frame.bind("<Configure>", lambda e: self.archive_canvas.configure(scrollregion=self.archive_canvas.bbox("all")))
        ...
        # --- Right: Finished ---
        finished_card = ttk.Frame(right, style="Finished.TFrame", padding=12)
        finished_card.grid(row=0, column=0, sticky="nsew", padx=12)
        ...
        finished_title = ttk.Label(finished_header, text="✓ Finished Tasks",
                                  font=("Segoe UI", 12, "bold"),
                                  foreground=APP_COLORS["success"])
        ...
        self.finished_canvas = tk.Canvas(self.finished_frame, borderwidth=0,
                                        highlightthickness=0, bg=APP_COLORS["bg_card"])
        self.finished_scroll = ttk.Scrollbar(self.finished_frame, orient="vertical", command=self.finished_canvas.yview)
        self.finished_list_frame = ttk.Frame(self.finished_canvas)
        self.finished_list_frame.bind("<Configure>", lambda e: self.finished_canvas.configure(scrollregion=self.finished_canvas.bbox("all")))
    ...
    # (render, create cards, archive, finished, details, etc...)


    def _render_finished(self):
        """Render finished tasks in the right panel"""
        for child in self.finished_list_frame.winfo_children():
            child.destroy()
        
        finished_tasks = [t for t in self.tasks if t.status == "done"]
        
        if not finished_tasks:
            no_finished_label = ttk.Label(self.finished_list_frame, 
                                        text="No finished tasks yet", 
                                        style="Finished.TLabel")
            no_finished_label.pack(pady=20)
            self.finished_count_label.config(text="✓ 0 Finished")
            return
        
        # Sort by creation date (most recent first)
        sorted_finished = sorted(finished_tasks, 
                               key=lambda t: t.created_at, 
                               reverse=True)
        
        self.finished_count_label.config(text=f"✓ {len(finished_tasks)} Finished")
    ...
    def _create_finished_item(self, task: Task, index: int):
        ...
        item_frame.columnconfigure(2, weight=0, minsize=60)
        ...
        # Task title (clickable) with truncation
        truncated_title = self._truncate_text(task.title, max_length=20)
        ...
        title_label.bind("<Button-1>", lambda e, t=task: self._show_task_details(t))
    ...
    def _create_archive_item(self, task: Task, index: int):
        ...
        item_frame.columnconfigure(0, weight=0, minsize=30)
        item_frame.columnconfigure(1, weight=1, minsize=80)
        item_frame.columnconfigure(2, weight=0, minsize=60)
        
        icon_label = ttk.Label(item_frame, text="🗂️", font=("Segoe UI", 10))
        icon_label.grid(row=0, column=0, sticky="w", padx=(0, 8))
        
        truncated_title = self._truncate_text(task.title, max_length=15)
        title_label = ttk.Label(item_frame, 
                              text=truncated_title, 
                              font=("Segoe UI", 9),
                              foreground=APP_COLORS["archive"],
        ...
        restore_btn = ttk.Button(btn_frame, text="↩️", 
                               width=2,
                               style="Archive.TButton",
                               command=lambda t=task: self._restore_task(t))
        restore_btn.grid(row=0, column=0, padx=1)
